...
List MCAStep2(NumericMatrix Z, NumericMatrix V, NumericVector Dc) {
    ...
    arma::mat samplesCoordinates =  AZ * AV;
    int AZcol = AZ.n_cols;
    AZ.clear();
    samplesCoordinates.each_col() %= ADc;
    ADc.clear();
    return List::create(Named("otusCoordinates") = wrap(std::sqrt(AZcol) * AV),
                        Named("samplesCoordinates") = wrap(samplesCoordinates.head_rows(samplesCoordinates.n_rows/2)));
}